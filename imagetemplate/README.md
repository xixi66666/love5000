...
- 亮色 AI 创作工作台：复用 website 首页 `03 Deep Woods` 的森林视频背景，外层工作区约 6% 雾白色膜并仅做约 1px 轻微模糊，结果区约 3%、卡片约 16%，以视频可见性优先；信息文字与标签统一使用近黑色层级，“加载更多提示词”与工作区使用相同透明玻璃；桌面端流程导航使用左侧居中的紧凑悬浮步骤器，窄屏恢复底部横排；灵感大厅采用顶部搜索、左侧分级筛选和右侧结果网格，后续保留模板解构 → Prompt 编导台 → 图片生成舱流程；模板 JSON 与变量剧本均可手工编辑。
