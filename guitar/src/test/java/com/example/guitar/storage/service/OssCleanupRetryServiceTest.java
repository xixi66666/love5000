...
class OssCleanupRetryServiceTest {
    ...
    void duePollClaimsEachPendingTaskBeforeDeletingAndContinuesAfterFailure() {
        ...
        when(dao.claimPending(eq(1L), any())).thenReturn(1); when(dao.claimPending(eq(2L), any())).thenReturn(1);
        ...
        verify(dao).markSuccess(eq(2L), any());
        verify(dao).reschedule(eq(1L), eq(1), any(), anyString(), any());
        ArgumentCaptor<String> error = ArgumentCaptor.forClass(String.class);
        verify(dao).reschedule(org.mockito.ArgumentMatchers.eq(1L), org.mockito.ArgumentMatchers.eq(1), any(), error.capture(), any());
    ...
    void failureBackoffAndFifthFailureArePersisted() {
        ...
        when(dao.claimPending(anyLong(), any())).thenReturn(1);
        OssUtil oss = mock(OssUtil.class); when(ossProvider.getIfAvailable()).thenReturn(oss);
        doThrow(new RuntimeException("x")).when(oss).delete(anyString());

        service.retryDueTasks();

        verify(dao).reschedule(org.mockito.ArgumentMatchers.eq(1L), org.mockito.ArgumentMatchers.eq(1),
                org.mockito.ArgumentMatchers.eq(LocalDateTime.of(2026, 7, 22, 2, 5)), anyString(), any());
        verify(dao).markFailed(org.mockito.ArgumentMatchers.eq(5L), org.mockito.ArgumentMatchers.eq(5), anyString(), any());
    }
    ...
    void absentOssReschedulesClaimedTaskAndStaleProcessingIsRecovered() {
        ...
        when(dao.claimPending(eq(9L), any())).thenReturn(1);
        when(ossProvider.getIfAvailable()).thenReturn(null);

        service.retryDueTasks();

        verify(dao).recoverStaleProcessing(any(), any());
        verify(dao).reschedule(org.mockito.ArgumentMatchers.eq(9L), org.mockito.ArgumentMatchers.eq(2),
                org.mockito.ArgumentMatchers.eq(LocalDateTime.of(2026, 7, 22, 2, 30)), anyString(), any());
    }

    private OssCleanupTask task(long id, String key, int retries) { OssCleanupTask t = new OssCleanupTask(); t.setId(id); t.setObjectKey(key); t.setRetryCount(retries); return t; }
