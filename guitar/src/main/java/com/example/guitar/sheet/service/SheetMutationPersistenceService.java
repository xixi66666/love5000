...
public class SheetMutationPersistenceService {
    ...
    public GuitarSheet updateMetadata(GuitarSheet current, SheetSaveRequest request) {
        ...
        applyMetadata(locked, request, false);
    ...
    @Transactional
    public void replaceFiles(GuitarSheet current, SheetSaveRequest request, List<GuitarSheetFile> files) {
        GuitarSheet locked = lock(current);
        applyMetadata(locked, request, true);
        if (sheetDao.updateMetadataAndFileMode(locked) != 1) throw saveFailed();
    ...
    private void applyMetadata(GuitarSheet target, SheetSaveRequest source, boolean includeFileMode) {
        ...
        if (includeFileMode) target.setFileMode(source.getFileMode().name());
    }
